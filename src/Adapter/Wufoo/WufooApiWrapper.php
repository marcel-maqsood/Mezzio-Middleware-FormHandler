...
class WufooApiWrapper extends WufooApiWrapperBase
{
    ...
    {
        $args = ['email' => $email, 'password' => $password, 'integrationKey' => $integrationKey, 'subdomain' => $subdomain];
    ...
    public function webHookPut($formIdentifier, $webHookUrl, $handshakeKey, $metadata = false)
    {
        ...
        $this->curl = new WufooCurl();
        $args = ['url' => $webHookUrl, 'handshakeKey' => $handshakeKey, 'metadata' => $metadata];
    ...
    public function webHookDelete($formIdentifier, $hash)
    {
        ...
        $this->curl = new WufooCurl();
        $result = json_decode($this->curl->deleteAuthenticated([], $url, $this->apiKey));
